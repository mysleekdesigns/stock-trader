"""XGBoost predictor with direction / return / volatility sub-models.

Mirrors the :class:`LightGBMPredictor` API so that models are
interchangeable throughout the trading system.
"""
...
class XGBoostPredictor(BaseModelPredictor):
    """Gradient-boosted tree predictor backed by XGBoost.
    ...
    * **volatility_regressor** – predicted realised volatility
    ...
    def __init__(self, params: dict[str, Any] | None = None) -> None:
        ...
        self.direction_classifier = XGBClassifier(**clf_params)
        self.return_regressor = XGBRegressor(**ret_params)
        self.volatility_regressor = XGBRegressor(**vol_params)

        self._early_stopping_rounds = user_params.get(
            "early_stopping_rounds", self._EARLY_STOPPING_ROUNDS
        )

        logger.info("xgboost_predictor_init", params=merged)
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame, **kwargs: Any) -> None:
        ...
        try:
            ...
            logger.info("xgboost_direction_trained")

            # --- return regressor ---
            self.return_regressor.fit(
                X, y["returns"], **fit_params["returns"]
            )
            logger.info("xgboost_return_trained")

            # --- volatility regressor ---
            self.volatility_regressor.fit(
                X, y["volatility"], **fit_params["volatility"]
            )
            logger.info("xgboost_volatility_trained")

            self.is_fitted = True
            logger.info("xgboost_training_complete")
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            raise ModelPredictionError(
                f"XGBoost predict_proba failed: {exc}",
                details={"original_error": str(exc)},
            ) from exc
    ...
    def load(self, path: Path) -> None:
        ...
        try:
            ...
            raise ModelLoadError(
                f"XGBoost model files not found at {path}",
